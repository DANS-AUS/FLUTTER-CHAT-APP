...
describe('User Routes', () => {
    ...
    test('POST / should create a new user', async () => {
        ...
        const userCheck : HydratedDocument<IUser> | null = await User.findOne({authId: user.authId});
    ...
    test('PUT /:id should update a user', async () => {
        ...
        const res = await request(app).put('/api/v1/users/123').send({ ...newUserInfo });
        
        const userCheck : HydratedDocument<IUser> | null = await User.findOne({authId: oldUser.authId});
